...
<div class="user-content-wrapper"
    style="background-image: @if(!empty(settings('login_logo')))  url('{{asset(path_image() . settings()['login_logo'])}}') @else url('{{asset('assets/user/images/user-content-wrapper-bg.jpg')}}') @endif">
            ...
            <div class="col-md-6">
                <div class="user-form">
                    <div class="user-form-inner">
                        ...
                        {{html()->form('POST')->attribute('id', 'submit-form')->route('loginProcess')->open()}}
                        ...
                        <div class="d-flex justify-content-between rememberme align-items-center mb-4">
                            <div class="text-right"><a class="text-theme forgot-password"
                                    href="{{route('forgotPassword')}}">{{__('Forgot Password?')}}</a>
                            </div>
                        </div>
                        @if (settings('select_captcha_type') == CAPTCHA_TYPE_GEETESTCAPTCHA)
                            <button type="button" class="btn btn-primary nimmu-user-sibmit-button"
                                id="submit-login">{{__('Sign In')}}</button>
                        @else
                            <button type="submit"
                                class="btn btn-primary nimmu-user-sibmit-button">{{__('Sign In')}}</button>
                        @endif
                        {{html()->form()->close()}}
                    </div>
                </div>
